...
def rename(base_directory):
    ...
    for root, subdirs, files in os.walk(base_directory):
        ...
        # Skip the base directory itself if there is sub directories
...
# manual_rename = tk.BooleanVar()
# tk.Checkbutton(window, text="Manual Rename", variable=manual_rename).pack()
# new_name = tk.Entry(window)
# new_name.pack(pady=10, padx=50, fill='x')

# def toggle_entry():
#     if manual_rename.get():
#         new_name.config(state='normal')
#     else:
#         new_name.config(state='disabled')

# manual_rename.trace_add('write', lambda *args: toggle_entry())
# toggle_entry() 
